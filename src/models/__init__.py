...
modules = [
    p.stem for p in Path(__file__).parent.iterdir() if p.suffix == ".py" and p.stem != "__init__"
]
to_expose = []

# Import all LightningDataModule's from the modules
for module_name in modules:
    module = importlib.import_module(f".{module_name}", "src.models")
    ...
    to_expose.extend(
        [
            ...
            and issubclass(getattr(module, name), LightningModule)
            and name != "LightningModule"
        ]
    )
