...
class SSTDataModule(pl.LightningDataModule):
    ...
    def __init__(self, batch_size, model_id):
        ...
        self.dataset_dict = datasets.load_dataset("sst").map(
            lambda x: self.tokenizer(
                x["sentence"], padding="max_length", max_length=65
            )
        ).map(
            # Add or remove epsilone if labels are 1 or 0 to avoid error in BCELoss
            lambda x: dict(
                label=x["label"] - 1e-6 if x["label"] == 1 else 
                x["label"] + 1e-6 if x["label"] == 0 else x["label"], 
